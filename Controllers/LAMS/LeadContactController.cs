...
namespace HaloBiz.Controllers.LAMS
{
    ...
    public class LeadContactController : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateById(long id, LeadContactReceivingDTO leadContactReceiving)
        {
            var response = await _leadContactService.UpdateLeadContact(HttpContext, id, leadContactReceiving);
            if (response.StatusCode >= 400)
                return StatusCode(response.StatusCode, response);
            var leadContact = ((ApiOkResponse)response).Result;
            return Ok(leadContact);
